...
권장 캘리브 절차:
    ...
    2. 스크립트 실행 → 자동으로 HOME 자세로 이동
    3. 다리 4개가 곧게 수직 아래로 펴진 자세여야 함
    4. 좌/우 또는 전/후 비대칭이 보이면 어긋난 관절을 +/- 키로 조정
    5. 4개 다리 모양이 모두 좌우 대칭, 전후 대칭이 되면 OK
    6. 'p' 키로 trim 출력 → SERVO_TRIMS 에 반영
"""
...
# MCU 의 HOME_ANGLES 와 동일 (다리 곧게 펴진 자세)
# Quadruped.cpp 의 HOME_ANGLES 와 반드시 일치시킬 것
HOME = [
    90.0,   0.0, 180.0,   # FL: hip=90 (중립), thigh=0 (수직 아래), calf=180 (직선)
    95.0, 168.0,   0.0,   # FR
    90.0,  10.0, 180.0,   # RL
    95.0, 180.0,  10.0,   # RR
]
