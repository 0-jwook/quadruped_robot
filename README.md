...
`gait_type='trot'` 모드에서:
- 일반 명령 (vx, vy ≠ 0) → **trot** 게이트
- **제자리 회전** (vx, vy ≈ 0, omega ≠ 0) → **wave** 자동 전환

이유: trot 은 dynamic balance 필요라 정지 회전엔 부적합. wave 는 항상 3-leg 지지로 정적 안정.
...
'max_stride':  0.025,    # half-stride L 의 상한 (m, 보폭 = 2L)
'period':      1.0,      # trot Tswing = period/2 = 0.5s,  wave Tswing = period/4 = 0.25s
'height_min':  0.07,     # body_height_cmd 의 최소값 (앉기 가능)
'height_max':  0.21,
'gait_type':   'trot',   # 'trot' (자동 wave 전환) 또는 '8phase'
'cmd_vel_hold_time': 30.0,  # cmd_vel 받은 뒤 stand 자세 복귀까지 시간 (s)
'pitch_offset': 0.0,     # 가상 pitch 보정 (rad). +값 = 앞 들기, -값 = 앞 내림
```
...
SERVO_TRIMS = {  # 도 단위
    'FL': ( 1.0,  19.0, -13.0),
    'FR': ( 7.0, -24.0,   4.0),   # 또는 -28 (변동 중)
    'RL': ( 0.0,  22.0,  -8.0),
    'RR': ( 6.0,  -8.0,  15.0),
}
```

큰 비대칭 (FR thigh -24 ~ -28) 이 정상. mechanical 조립 오차를 SW 로 보정 중.
...
| 좌측 휨 | ⚠️ 잔여 | 캘리브 잔여 비대칭. FORWARD_BOOST 1.15 로 일부 보정 |
| 측방 보행 | ⚠️ 약함 | trot의 본질적 한계 |
| 패킷 손실 (PKT:16~32) | ⚠️ 가끔 | 서보 노이즈 → I2C BUSY → 메인 루프 블로킹 |
| IMU 피드백 | ❌ 미설치 | 코드는 있음, 센서 안 달면 dz=0 |
...
| 좌측 휨 | 좌우 비대칭 캘리브 | `tools/calibrate.py` 재실행 |
| 회전 시 뒤로 | 좌우 추진력 비대칭 | `FORWARD_BOOST` 키움 또는 캘리브 |
| 발 끌림 | step_height 너무 낮음 | `step_height` 0.03 → 0.045 |
| 너무 빠름 | period 짧음 | `period` 1.0 → 1.3+ |
| 너무 느림 | period 김 | `period` 1.0 → 0.7 |
| 비행 구간 / 흔들림 | duty < 0.5 | `tstance_min_ratio` 키움 (현재 3.0) |
